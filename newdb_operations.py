...
def upsert_drive_fragment(file_id, fragment_index, drive_file_id):
    conn, cursor = get_connection()
    if not conn: return
    try:
        cursor.execute("""
            INSERT INTO drive_fragments (file_id, fragment_index, drive_file_id)
            VALUES (%s, %s, %s)
            ON DUPLICATE KEY UPDATE drive_file_id=VALUES(drive_file_id)
        """, (file_id, fragment_index, drive_file_id))
        conn.commit()
    finally:
        cursor.close(); conn.close()

def list_drive_fragments(file_id):
    conn, cursor = get_connection()
    if not conn: return []
    try:
        cursor.execute("""
            SELECT fragment_index, drive_file_id
            FROM drive_fragments
            WHERE file_id=%s
